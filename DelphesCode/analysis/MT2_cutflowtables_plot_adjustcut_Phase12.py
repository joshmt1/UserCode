...
#args: <NMn where n=1,2,3>

#event counts for 300fb-1 are hard-coded below

#this version of the code uses:
#PhaseI for lumi 0-300
#PhaseI+II for lumi >300
...
#we want to use only the 500,600,800 points
...
n_sm_1 = [ n_sm_1[1], n_sm_1[2],n_sm_1[4]]
n_sm_2 = [ n_sm_2[1], n_sm_2[2],n_sm_2[4]]
n_nm1_1 = [ n_nm1_1[1], n_nm1_1[2],n_nm1_1[4]]
n_nm2_1 = [ n_nm2_1[1], n_nm2_1[2],n_nm2_1[4]]
n_nm3_1 = [ n_nm3_1[1], n_nm3_1[2],n_nm3_1[4]]
n_nm1_2 = [ n_nm1_2[1], n_nm1_2[2],n_nm1_2[4]]
n_nm2_2 = [ n_nm2_2[1], n_nm2_2[2],n_nm2_2[4]]
n_nm3_2 = [ n_nm3_2[1], n_nm3_2[2],n_nm3_2[4]]


def zbi(b,s,f):
    return RooStats.NumberCountingUtils.BinomialObsZ(b+s,b,f)

def get_graph( sm300, sig300, db, sm300_2,sig300_2):
#_2 are the phase2 sm, signal
    g1 = TGraph()
    g2 = TGraph()
    ...
    for ip,lumi in enumerate(range(10,3001,10)):
        ...
        bestfom = -99
        bestcut = -99
        for ism,ism2,isig,isig2,icut in zip(sm300,sm300_2,sig300,sig300_2,cutval):
            #here, calculate sm and sig for the given lumi
            if lumi<=300:
                this_sm = scale*ism
                this_sig= scale*isig
            else:
                scale2 = (float(lumi)-300)/float(300)
                this_sm = ism + scale2*ism2
                this_sig= isig+ scale2*isig2
            fom = zbi(this_sm,this_sig,db)
            if fom>bestfom:
                bestfom=fom
                bestcut=icut
...
(g_550,cut_550) = get_graph(n_sm_1, S1, 0.3, n_sm_2,S2)
g_550.SetName('mt2_550')
cut_550.SetName('mt2_550_bestcut')

#sys.exit(1)

(g_550_0p15,cut_550_0p15) = get_graph(n_sm_1, S1, 0.15,n_sm_2,S2)
g_550_0p15.SetName('mt2_550_0p15')
cut_550_0p15.SetName('mt2_550_0p15_bestcut')

(g_550_0p50,cut_550_0p50) = get_graph(n_sm_1, S1, 0.5 ,n_sm_2,S2)
...
filename='DelphesMT2/MT2_significance_Phase12_'+which+'.root'
